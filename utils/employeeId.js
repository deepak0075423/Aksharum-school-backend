'use strict';
/**
 * Employee / Teacher IDs follow their own per-school template, separate from the
 * admission-number format, e.g. "{INITIALS}{####}" → KEHS0001 for
 ...
 * {YYYY} restarts the count each academic year.
 */
const AcademicYear   = require('../models/AcademicYear');
const TeacherProfile = require('../models/TeacherProfile');

const DEFAULT_FORMAT = '{INITIALS}{####}';
const SEQ_TOKEN = /\{(#+|SEQ)\}/;
...
function validateFormat(format) {
    ...
    if (!value) return 'Employee ID format is required';
    if (value.length > 40) return 'Employee ID format is too long';
    ...
    const unknown = (value.match(/\{[^}]*\}/g) || [])
        .filter(t => !/^\{(INITIALS|CODE|YYYY|YY|MM|DD|#+|SEQ)\}$/.test(t));
    if (unknown.length)
        return `Unknown token ${unknown[0]} — use {INITIALS}, {CODE}, {YYYY}, {YY}, {MM}, {DD} or {####}`;
    return null;
}
 ...
 */
function parseFormat(format, { school, year, on = new Date() }) {
...
/** What the next ID would look like — used by the settings preview. */
async function previewEmployeeId(format, school, seq = 1) {
    const year = await resolveYear(school?._id);
    return compose(parseFormat(format, { school, year }), seq);
}

/**
 * Next free employee ID for a school. Derived from the highest existing ID
 * sharing the same prefix, so it stays correct across restarts, and skips over
 * anything already assigned by hand.
 *
 * @param {Object} school school document (needs _id, name, code, employeeIdFormat)
 * @returns {Promise<String>}
 */
async function nextEmployeeId(school) {
    const year  = await resolveYear(school?._id);
    const parts = parseFormat(school?.employeeIdFormat || DEFAULT_FORMAT, { school, year });

    const existing = await TeacherProfile.find(
        { school: school._id, employeeId: { $ne: '' } }, 'employeeId',
    ).lean();

    const taken = new Set(existing.map(p => String(p.employeeId || '')).filter(Boolean));
    let highest = 0;
...
module.exports = {
    ...
    previewEmployeeId, nextEmployeeId, resolveYear,
};
